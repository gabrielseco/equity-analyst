...
async function testTickerResolution() {
  ...
  if (!apiKey) {
    console.error('Error: No Alpha Vantage API key found. Set ALPHA_VANTAGE_API_KEY environment variable.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`✅ Result: ${result1.symbol} - ${result1.name} (isResolved: ${result1.isResolved})\n`);
  } catch (error) {
  ...
  try {
    ...
    console.log(`✅ Result: ${result2.symbol} - ${result2.name} (isResolved: ${result2.isResolved})\n`);
  } catch (error) {
    console.error(`❌ Error: ${error}\n`);
  }
  ...
  try {
    ...
    console.log(`✅ Result: ${result3.symbol} - ${result3.name} (isResolved: ${result3.isResolved})\n`);
  } catch (error) {
    console.error(`❌ Error: ${error}\n`);
  }
